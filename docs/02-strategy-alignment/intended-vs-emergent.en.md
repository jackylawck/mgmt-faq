...
```mermaid
graph LR
    A[Intended Strategy] --> B{Environmental Filter};
    B -->|Unrealized Strategy| C[Abandoned Plans];
    B -->|Deliberate Strategy| D((Realized Strategy));
    E[Emergent Strategy] -->|Frontline Innovation| D;
